...
using System; // Required for Guid.NewGuid()
  ...
  public class UsersController : Controller
  {
    ...
    public async Task<IActionResult> CreateTenant(TenantCreateViewModel model)
    {
      if (ModelState.IsValid)
      {
        ...
        {
          Id = Guid.NewGuid().ToString(), // Assign a new GUID string for the Tenant ID
          Name = model.Name,
        ...
        try
        {
          ...
          return RedirectToAction(nameof(Index)); // Redirect to the user list or a dedicated tenant list if you create one
        }
        catch (DbUpdateException ex)
        {
          // Handle potential database errors (e.g., if a unique constraint on Name exists)
      ...
      // If ModelState is not valid or an error occurred, return the view with errors.
