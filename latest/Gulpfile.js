...
// Build Site CSS (your custom styles)
// -------------------------------------------------------------------------------
const buildSiteCssTask = function () {
  // 'root' function resolves paths relative to src/
  const siteSassPath = root('site.scss'); // This correctly targets C:/Users/HAFIZI/source/repos/Combined version 12-6-2025/src/site.scss

  // This determines the output directory: C:/Users/HAFIZI/source/repos/Combined version 12-6-2025/wwwroot/css
  const outputDir = path.join(conf.distPath, 'css');

  return src(siteSassPath)
    .pipe(
      sass.sync({ // Use gulp-dart-sass directly for simplicity and reliability here
        includePaths: ['node_modules'], // Ensure node_modules are included for @import
        outputStyle: conf.minify ? 'compressed' : 'expanded' // Use minified or expanded based on your config
      }).on('error', sass.logError) // Log any Sass compilation errors
    )
    .pipe(autoprefixer()) // Add vendor prefixes for browser compatibility
    .pipe(rename('site.css')) // Ensure the output file is always named site.css
    .pipe(dest(outputDir)) // Output the compiled CSS to the target directory
    .pipe(browserSync.stream()); // If you're using BrowserSync for live reloads
};
// Build (Dev & Prod)
// -------------------------------------------------------------------------------
const buildJsTask = series(webpackJsTask, pageJsTask);

const buildTasks = [buildCssTask, buildSiteCssTask, buildJsTask, buildFontsTask];
...
module.exports = {
  ...
  'build:site-css': buildSiteCssTask, // <-- Add this line
