...
namespace AspnetCoreMvcFull.Controllers
{
  public class DocumentRepositoryController : Controller
  {
    ...
    // POST: DocumentRepository/UploadDocument
    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> UploadDocument(UploadDocumentViewModel model)
    {
      if (!ModelState.IsValid)
      {
        // If model state is not valid, re-populate folder name and required doc name for the view
        var folder = await _context.ComplianceFolders.FindAsync(model.ComplianceFolderId);
        if (folder != null)
        {
          model.ComplianceFolderName = folder.Name;
        }
        if (model.RequiredDocumentId.HasValue)
        {
          var requiredDoc = await _context.RequiredDocuments.FindAsync(model.RequiredDocumentId.Value);
          if (requiredDoc != null)
          {
            model.RequiredDocumentName = requiredDoc.DocumentName;
          }
        }
        return View(model);
      }

      var complianceFolder = await _context.ComplianceFolders.FindAsync(model.ComplianceFolderId);
      if (complianceFolder == null)
      {
      ...
      foreach (var file in model.Files)
      {
        if (file != null && file.Length > 0)
        {
          try
          {
            ...
            var uploadsFolder = Path.Combine(_environment.WebRootPath, "uploads", _tenantService.GetCurrentTenantId().ToString(), "documents");
            ...
            var uploadedByUserName = _httpContextAccessor.HttpContext?.User?.Identity?.Name ?? "System";

            // Create a new Document entity
            var document = new Document
            {
              FileName = file.FileName,
              FilePath = "/uploads/" + _tenantService.GetCurrentTenantId().ToString() + "/documents/" + uniqueFileName, // Store relative path
              FileType = GetContentType(Path.GetExtension(file.FileName)),
              FileSize = file.Length,
              Description = model.Description ?? string.Empty,
              UploadDate = DateTime.Now,
              UploadedBy = uploadedByUserName,
              Status = DocumentStatus.Active, // Default status
              ComplianceFolderId = model.ComplianceFolderId,
              RequiredDocumentId = model.RequiredDocumentId // Assign if it's a required document upload
            };

            _context.Documents.Add(document);
            uploadedCount++;

            // If it's a required document upload, update its status
            if (model.RequiredDocumentId.HasValue)
            {
              var requiredDoc = await _context.RequiredDocuments.FindAsync(model.RequiredDocumentId.Value);
              if (requiredDoc != null)
              {
                ...
                // Add the newly uploaded document to the RequiredDocument's Documents collection
                requiredDoc.Documents.Add(document);
      ...
      {
        await _context.SaveChangesAsync();
        TempData["SuccessMessage"] = $"Successfully uploaded {uploadedCount} document(s).";
        return RedirectToAction("FolderDetails", new { id = model.ComplianceFolderId });
      }
      else
      {
        TempData["ErrorMessage"] = "No files were uploaded. Please select at least one file.";
        // If no files were uploaded due to errors or no selection, return to view
        var folder = await _context.ComplianceFolders.FindAsync(model.ComplianceFolderId);
        if (folder != null)
        {
          model.ComplianceFolderName = folder.Name;
