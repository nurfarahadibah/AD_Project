...
namespace AspnetCoreMvcFull.Services
{
    public class DashboardService : IDashboardService
    {
        ...
        public async Task<DashboardViewModel> GetDashboardDataAsync(ApplicationUser? user)
        {
            ...
            viewModel.TotalComplianceFolders = await _context.ComplianceFolders.CountAsync();
            viewModel.ActiveComplianceFolders = await _context.ComplianceFolders.CountAsync(f => f.Status == FolderStatus.Active);
            viewModel.ArchivedComplianceFolders = await _context.ComplianceFolders.CountAsync(f => f.Status == FolderStatus.Archived);

            viewModel.TotalDocuments = await _context.Documents.CountAsync();
            viewModel.PendingReviewDocuments = await _context.Documents.CountAsync(d => d.Status == DocumentStatus.PendingReview);
            viewModel.ApprovedDocuments = await _context.Documents.CountAsync(d => d.Status == DocumentStatus.Approved);

            viewModel.TotalRequiredDocuments = await _context.RequiredDocuments.CountAsync();
            viewModel.SubmittedRequiredDocuments = await _context.RequiredDocuments.CountAsync(rd => rd.IsSubmitted);
            viewModel.OutstandingRequiredDocuments = await _context.RequiredDocuments.CountAsync(rd => !rd.IsSubmitted);

            viewModel.ComplianceCategoryCounts = await _context.ComplianceCategories
               .Select(cc => new { cc.Name, Count = cc.ComplianceFolders.Count() })
