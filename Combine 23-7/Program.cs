...
app.MapControllerRoute(
    ...
    pattern: "{controller=SuperAdminDashboard}/{action=Index}/{id?}");
...
using (var scope = app.Services.CreateScope())
{
  ...
  try
  {
    ...
    if (superadminUser == null)
    {
      ...
      var result = await userManager.CreateAsync(superadminUser, "Superadmin1*");
      if (result.Succeeded)
      {
    ...
    // Manager user for Tenant B (Existing)
    string managerEmailB = "managerB@example.com";
    string managerPasswordB = "Test1234,";
    if (await userManager.FindByEmailAsync(managerEmailB) == null)
    {
      var managerUserB = new ApplicationUser { UserName = managerEmailB, Email = managerEmailB, TenantId = "tenantB" };
      var result = await userManager.CreateAsync(managerUserB, managerPasswordB);
      if (result.Succeeded)
      {
        await userManager.AddToRoleAsync(managerUserB, "Manager");
      }
      else { Console.WriteLine($"Failed to create managerUserA: {string.Join(", ", result.Errors.Select(e => e.Description))}"); }
    }
    ...
    // Regular user for Tenant A (Existing)
    string userEmailA = "userA@example.com";
    string userPasswordA = "Test1234,";
    if (await userManager.FindByEmailAsync(userEmailA) == null)
    {
      var regularUserA = new ApplicationUser { UserName = userEmailA, Email = userEmailA, TenantId = "tenantA" };
      var result = await userManager.CreateAsync(regularUserA, userPasswordA);
      if (result.Succeeded)
      {
        await userManager.AddToRoleAsync(regularUserA, "User");
      }
      else { Console.WriteLine($"Failed to create regularUserB: {string.Join(", ", result.Errors.Select(e => e.Description))}"); }
    }
