...
namespace AspnetCoreMvcFull.Controllers
{
  public class FormBuilderController : Controller
  {
    ...
    // Action to completely delete a form (still uses AJAX in Index.cshtml for confirmation)
    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> DeleteFormCompletely(int id)
    {
      ...
      catch (Exception ex)
      {
        ...
        // If an error occurs, return a 500 status code with a plain text message.
        // The client-side JS (deleteFormCompletely) handles this text response.
        return StatusCode(500, "An error occurred while completely deleting the form.");
      }
    }
