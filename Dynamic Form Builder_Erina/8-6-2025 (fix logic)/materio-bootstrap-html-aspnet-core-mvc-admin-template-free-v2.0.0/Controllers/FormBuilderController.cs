...
namespace AspnetCoreMvcFull.Controllers
{
  public class FormBuilderController : Controller
  {
    ...
    public async Task<IActionResult> Index(int? complianceCategoryId, FormStatus? formStatus, string sortOrder) // Added sortOrder parameter
    {
      ...
      // If formStatus is null (meaning "All" was selected or no filter is active),
      // we will NOT add a status-based WHERE clause, thus including all statuses.
      if (formStatus.HasValue)
      {
        jenisFormsQuery = jenisFormsQuery.Where(f => f.Status == formStatus.Value);
      }
      // The 'else' block that filtered out archived forms is removed/commented out.
      // This ensures that when formStatus.HasValue is false (e.g., from the "All" tab),
      // no status filter is applied, and all forms including archived ones are returned.
      // --- END CRITICAL CHANGE ---
