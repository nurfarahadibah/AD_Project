...
using System.Threading.Tasks; // Ensure this is present for async methods
...
namespace AspnetCoreMvcFull.Controllers
{
  public class FormBuilderController : Controller
  {
    ...
    public async Task<IActionResult> Index(int? complianceCategoryId, FormStatus? formStatus, string sortOrder) // Added sortOrder parameter
    {
      ...
      // MODIFIED: Added .Where(c => c.Status == 1) to only show active categories
      ViewBag.ComplianceCategories = await _context.ComplianceCategories
          .Where(c => c.Status == 1) // Filter for active compliance categories
    ...
    // NEW: GradeConfiguration Action (GET)
    [HttpGet]
    public async Task<IActionResult> GradeConfiguration(int formTypeId)
    {
      var jenisForm = await _context.JenisForms
          .Include(f => f.GradeConfiguration)
              .ThenInclude(gc => gc.GradeRanges.OrderBy(gr => gr.Order)) // Order by 'Order' property
          .FirstOrDefaultAsync(f => f.FormTypeId == formTypeId);
      ...
      var viewModel = new GradeConfigurationViewModel
      {
        FormTypeId = formTypeId,
        GradeRanges = jenisForm.GradeConfiguration?.GradeRanges
                      .Select(gr => new GradeRangeViewModel
                      {
                        MinPercentage = gr.MinPercentage,
                        MaxPercentage = gr.MaxPercentage,
                        GradeLetter = gr.GradeLetter
                      })
                      .ToList() ?? new List<GradeRangeViewModel>()
      };

      ViewBag.FormName = jenisForm.Name; // Pass form name to the view
      return View(viewModel);
    }


    // Modified SaveGradeConfiguration Action (POST)
    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> SaveGradeConfiguration([FromBody] GradeConfigurationViewModel model)
    {
      if (!ModelState.IsValid)
      {
      ...
      // Basic validation for overlapping ranges and completeness
      var sortedRanges = model.GradeRanges.OrderBy(r => r.MinPercentage).ToList();
      for (int i = 0; i < sortedRanges.Count; i++)
      {
        if (sortedRanges[i].MinPercentage > sortedRanges[i].MaxPercentage)
        {
          return BadRequest(new { success = false, message = $"Min Percentage ({sortedRanges[i].MinPercentage}) cannot be greater than Max Percentage ({sortedRanges[i].MaxPercentage}) for grade letter '{sortedRanges[i].GradeLetter}'." });
        }
        if (sortedRanges[i].MinPercentage < 0 || sortedRanges[i].MaxPercentage > 100)
        {
          return BadRequest(new { success = false, message = "Percentage values must be between 0 and 100." });
        }
        if (i < sortedRanges.Count - 1 && sortedRanges[i].MaxPercentage >= sortedRanges[i + 1].MinPercentage)
        {
          return BadRequest(new { success = false, message = $"Grade ranges overlap or are not continuous. Check ranges for '{sortedRanges[i].GradeLetter}' and '{sortedRanges[i + 1].GradeLetter}'." });
        }
      }

      var gradeConfig = await _context.GradeConfigurations
          .Include(gc => gc.GradeRanges)
          .FirstOrDefaultAsync(gc => gc.FormTypeId == model.FormTypeId);

      if (gradeConfig == null)
      {
        // Create new GradeConfiguration if it doesn't exist
      ...
      {
        // Clear existing ranges and update LastModifiedAt
        _context.GradeRanges.RemoveRange(gradeConfig.GradeRanges);
        gradeConfig.GradeRanges.Clear();
        gradeConfig.LastModifiedAt = DateTime.Now;
      }

      int order = 1;
      foreach (var rangeModel in sortedRanges)
      {
        gradeConfig.GradeRanges.Add(new GradeRange
        {
          MinPercentage = rangeModel.MinPercentage,
          MaxPercentage = rangeModel.MaxPercentage,
          GradeLetter = rangeModel.GradeLetter,
          Order = order++
        });
      }
      ...
      {
        await _context.SaveChangesAsync();
        // Redirect to the GradeConfiguration page after successful save
        return Ok(new { success = true, message = "Grading configuration saved successfully!", redirectToUrl = Url.Action("GradeConfiguration", new { formTypeId = model.FormTypeId }) });
      }
      catch (Exception ex)
      {
        Console.Error.WriteLine($"Error saving grading configuration for FormTypeId {model.FormTypeId}: {ex.Message}");
        return StatusCode(500, new { success = false, message = "An error occurred while saving the grading configuration. Please try again." });
      }
    }
