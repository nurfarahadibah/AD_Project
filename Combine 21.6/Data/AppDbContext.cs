...
namespace AspnetCoreMvcFull.Data
{
  public class AppDbContext : DbContext
  {
    ...
    public DbSet<GradeConfiguration> GradeConfigurations { get; set; } // Added
    public DbSet<GradeRange> GradeRanges { get; set; } // Added
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
      ...
      // Configure One-to-One relationship between JenisForm and GradeConfiguration
      // FormTypeId in GradeConfiguration is both primary key and foreign key to JenisForm
      modelBuilder.Entity<JenisForm>()
          .HasOne(jf => jf.GradeConfiguration)
          .WithOne(gc => gc.JenisForm)
          .HasForeignKey<GradeConfiguration>(gc => gc.FormTypeId); // One-to-one with shared primary key

      // Configure One-to-Many relationship between GradeConfiguration and GradeRange
      modelBuilder.Entity<GradeConfiguration>()
          .HasMany(gc => gc.GradeRanges)
          .WithOne(gr => gr.GradeConfiguration)
          .HasForeignKey(gr => gr.GradeConfigurationId); // One-to-many relationship

      // Existing configurations from the provided snippet
      modelBuilder.Entity<CorrectiveAction>()
                .HasOne(ca => ca.AuditResponse)
                .WithMany(ar => ar.CorrectiveActions)
                .HasForeignKey(ca => ca.AuditResponseId)
                .OnDelete(DeleteBehavior.Restrict);
